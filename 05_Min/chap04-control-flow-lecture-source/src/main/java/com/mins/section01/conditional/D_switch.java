package com.mins.section01.conditional;
...
public class D_switch {
  ...
  public void testSimpleSwitchStatement() {
    ...
    switch (op) {
      ...
      case '-' : result = first - second; break;
      case 'x' : System.out.println(" x 선택됨"); // break 가 없으면 다음 case로 이동
      case '*' : result = first * second; break;   // ← 수정: 빈 문자('') 대신 '*' 로 변경
      case '/' : result = first / second; break;
      case '%' : result = first % second; break;
      default :
  ...
  public void improvetestSimpleSwitchStatement() {
    ...
    int result = switch (op) {
      ...
      case 'x', '*', 'X' -> first * second;   // ← 수정: '' 제거 + 곱하기로 정상 처리
      case '/' -> first / second;
      case '%' -> first % second;
      default -> {
        System.out.println("잘못된 연산자 입니다.");
